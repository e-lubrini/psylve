...
# extract text from file with ocr
    ...
    if processing_rate > 5000: # if rate is too high, documents so far haven't needed any processing, so timing for estimation is reset until first processable document is reached by loop
        size_done = 0
        start_time = time.time()
    else:
